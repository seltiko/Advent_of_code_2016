...
bot 96 gives low to bot 10 and high to bot 152'::text as inputs
;
select * from temp_input;
drop table if exists test_input;
create temp table test_input as
select 
...
value 2 goes to bot 2'::text as inputs
;


drop table if exists starting;
create temp table starting as
select 
    bot
    ,min(value1) as value1
    ,case when count(1) = 1 then null else max(value1) end as value2
    ,inputs
    ,count(1) as num
    ,0 as iter
    ,1 as bot_flg
from(
select t.inst
    ,substring(inst,'\d+$') as bot
    ,substring(inst,'\d+')::int as value1
    ,null::text as value2
    ,inputs    
from(
...
inner join temp_input t2
--inner join test_input t2
    on true
where inst ~'^value'
    )t3
group by 1,4,6,7
...
create or replace function pg_temp.passon() returns void as $$
insert into starting
select 
    bot
    ,min(value1) as value1
    ,case when count(1) = 1 then null else max(value1) end as value2
...
select * 
from starting
where (num <> 2 or bot_flg = 0)
    and iter = (select max(iter) from starting)
union
select substring(inputs,'bot '||bot||' gives low to \w+ (\d+) and high to \w+ \d+') as bot
    ,least(value1,value2) as value1
    ,null as value2
    ,inputs
    ,1 as num
    ,iter
    ,case when substring(inputs,'bot '||bot||' gives low to (\w+) \d+ and high to \w+ \d+') = 'bot' then 1 else 0 end as bot_flg
from starting
where num = 2
    and iter = (select max(iter) from starting)
    and bot_flg = 1
union
select substring(inputs,'bot '||bot||' gives low to \w+ \d+ and high to \w+ (\d+)') as bot
    ,greatest(value1,value2) as value1
    ,null as value2
    ,inputs
    ,1 as num
    ,iter
...
where exists(
            select 1
            from starting s
            where s.iter = (select max(iter) from starting s2)
                and s.bot_flg = 1
        )
group by 1,4,6,7
;
...
language sql;
/*
select pg_temp.passon() where exists(
            select 1
            from starting s
            where s.iter = (select max(iter) from starting s2)
                and s.bot_flg = 1
        );
select * from starting where iter = 1;
select pg_temp.passon();
select * from starting where iter = 2 and bot::int in(153,20,180,72,93,180);
select exists(
            select 1
            from starting s
            where s.iter = (select max(iter) from starting s2)
...
with recursive pass(passon,iter)as
(
select null::void as passon,0 as iter

union all
select pg_temp.passon(), iter + 1 as iter
from pass
    where iter < 39
...
select * from starting;
select * from starting where (value1::int = 17 and value2::int = 61)
    or (value1::int = 61 and value2::int = 17);

select *
from starting
where bot_flg = 0
    and bot::int in (0,1,2)
    and iter = 39
