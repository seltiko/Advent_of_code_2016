...
bwzsacxgqkbjycgfw[dbnligvrmqscasutn]rbgybqqsgjvlonkut'::text as inputs
;


with parsed as(

select row_number() over(partition by id) as split_num
	,*
-- 	,regexp_matches(splits,'(\w)(\w)\2\1')
-- 	,array_to_string(regexp_matches(splits,'(\w)(\w)\2\1'),'','')
-- 	,regexp_replace(array_to_string(regexp_matches(splits,'(\w)(\w)\2\1'),'',''),'(\w)\1','')
	,regexp_replace(array_to_string(regexp_matches(splits,'(\w)(\w)\2\1'),'',''),'(\w)\1','') <> '' as target
from(
select row_number() over() as id
	,*
-- 	,regexp_replace(array_to_string(regexp_matches(inst,'(\w)(\w)\2\1'),'',''),'(\w)\1\1\1','') <> ''
	,regexp_split_to_table(inst, '[\[\]]') as splits
-- 	,(regexp_split_to_array(inst, '[\[\]]'))[1] as split1
from(
select regexp_split_to_table(inputs,'\n') as inst
from temp_input
-- limit 100
)t
where inst ~* '([a-z])([a-z])\2\1'
-- 	and regexp_replace(array_to_string(regexp_matches(inst,'(\w)(\w)\2\1'),'',''),'(\w)\1\1\1','') <> ''
-- 	and inst ~* '\]\['
)t
)
...
-- 	,split_num
	,inst
-- 	,splits
-- 	,(splits ~* '([a-z])([a-z])\2\1')::int
from parsed p
where not exists(
	select 1
	from parsed p9
	where p9.id = p.id
		and p9.split_num%2=0
-- 		and p9.splits ~* '([a-z])([a-z])\2\1'
		and p9.target
	)
	and exists(
	select 1
	from parsed p9
	where p9.id = p.id
		and p9.split_num%2=1
-- 		and p9.splits ~* '([a-z])([a-z])\2\1'
		and p9.target
	)
-- 	and splits ~* '([a-z])([a-z])\2\1'
order by 1,2
;
...
select row_number() over(partition by id) as split_num
	,*
	,regexp_matches(splits,'(\w)(\w)(\1)','g')
	,regexp_matches(reverse(splits),'(\w)(\w)(\1)','g') as reverse_reverse
	,position(left(array_to_string((regexp_matches(splits,'(\w)(\w)(\1)')),'',''),2) in splits) as forward_pos
	,position(left(array_to_string((regexp_matches(reverse(splits),'(\w)(\w)(\1)')),'',''),2) in reverse(splits)) as reverse_pos
-- 	,array_to_string(regexp_matches(splits,'(\w)(\w)(\1)','g'),'','')
-- 	,regexp_replace(array_to_string(regexp_matches(splits,'(\w)(\w)(\1)','g'),'',''),'(\w)\1\1','')
-- 	,regexp_replace(array_to_string(regexp_matches(splits,'(\w)(\w)(\1)','g'),'',''),'(\w)\1\1','') <> '' as target
from(
select row_number() over() as id
	,*
-- 	,regexp_replace(array_to_string(regexp_matches(inst,'(\w)(\w)\2\1'),'',''),'(\w)\1\1\1','') <> ''
...
select *
	,regexp_matches(substring(splits from forward_pos+1 for (char_length(splits)-forward_pos-1)),'(\w)(\w)(\1)') as short_for
	,regexp_matches(substring(reverse(splits) from reverse_pos+1 for (char_length(splits)-reverse_pos-1)),'(\w)(\w)(\1)') as short_rev
from(
select row_number() over(partition by id) as split_num
	,*
	,regexp_matches(splits,'(\w)(\w)(\1)','g')
...
-- 	,p.target
from parsedfinal p
inner join parsedfinal p2
	on p2.id = p.id
-- 	and p2.target
	and p2.split_num%2 = 0
	and(
	(p.regexp_matches[1] = p2.regexp_matches[2]
	and p2.regexp_matches[1] = p.regexp_matches[2]
	and p.regexp_matches[1] <> p.regexp_matches[2])
	or
	(p.reverse_reverse[1] = p2.reverse_reverse[2]
	and p2.reverse_reverse[1] = p.reverse_reverse[2]
	and p.reverse_reverse[1] <> p.reverse_reverse[2])
	or
	(p.short_for[1] = p2.short_for[2]
	and p2.short_for[1] = p.short_for[2]
	and p.short_for[1] <> p.short_for[2])
	or
	(p.short_rev[1] = p2.short_rev[2]
	and p2.short_rev[1] = p.short_rev[2]
	and p.short_rev[1] <> p.short_rev[2])
	or
	(p.regexp_matches[1] = p2.reverse_reverse[2]
	and p2.reverse_reverse[1] = p.regexp_matches[2]
	and p.regexp_matches[1] <> p.regexp_matches[2])
	or
	(p.regexp_matches[1] = p2.short_for[2]
	and p2.short_for[1] = p.regexp_matches[2]
	and p.regexp_matches[1] <> p.regexp_matches[2])
	or
	(p.regexp_matches[1] = p2.short_rev[2]
	and p2.short_rev[1] = p.regexp_matches[2]
	and p.regexp_matches[1] <> p.regexp_matches[2])
	or
	(p.reverse_reverse[1] = p2.regexp_matches[2]
	and p2.regexp_matches[1] = p.reverse_reverse[2]
	and p.reverse_reverse[1] <> p.reverse_reverse[2])
	or
	(p.reverse_reverse[1] = p2.short_for[2]
	and p2.short_for[1] = p.reverse_reverse[2]
	and p.reverse_reverse[1] <> p.reverse_reverse[2])
	or
	(p.reverse_reverse[1] = p2.short_rev[2]
	and p2.short_rev[1] = p.reverse_reverse[2]
	and p.reverse_reverse[1] <> p.reverse_reverse[2])
	or
	(p.short_for[1] = p2.regexp_matches[2]
	and p2.regexp_matches[1] = p.short_for[2]
	and p.short_for[1] <> p.short_for[2])
	or
	(p.short_for[1] = p2.reverse_reverse[2]
	and p2.reverse_reverse[1] = p.short_for[2]
	and p.short_for[1] <> p.short_for[2])
	or
	(p.short_for[1] = p2.short_rev[2]
	and p2.short_rev[1] = p.short_for[2]
	and p.short_for[1] <> p.short_for[2])
	or
	(p.short_rev[1] = p2.regexp_matches[2]
	and p2.regexp_matches[1] = p.short_rev[2]
	and p.short_rev[1] <> p.short_rev[2])
	or
	(p.short_rev[1] = p2.reverse_reverse[2]
	and p2.reverse_reverse[1] = p.short_rev[2]
	and p.short_rev[1] <> p.short_rev[2])
	or
	(p.short_rev[1] = p2.short_for[2]
	and p2.short_for[1] = p.short_rev[2]
	and p.short_rev[1] <> p.short_rev[2])
	)
...
-- 	and p.target
	and p.split_num%2 = 1
-- 	and not( p2.regexp_matches[1] = p.regexp_matches[2])
-- 	and exists(--still need or parsed2
-- 		select 1
-- 		from parsed p9
-- 		where p9.id = p.id
-- 			and p9.split_num%2 = 1
